import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import MainLayoutTeacher from "../../components/Teacher/MainLayout";

const IconAcademic = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5z" />
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
  </svg>
);

const IconPlusCircle = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <circle cx="12" cy="12" r="9" strokeWidth="2" />
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v8M8 12h8" />
  </svg>
);

export default function DashboardTeacher() {
  const navigate = useNavigate();
  const [stats, setStats] = useState({ total_classes: 0, total_students: 0, total_assignments: 0 });
  const [upcomingClasses, setUpcomingClasses] = useState([]);
  const [todaySchedule, setTodaySchedule] = useState([]);
  const [recentSubmissions, setRecentSubmissions] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  const fetchStats = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("/api/teacher/statistics", {
        method: "GET",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420"
        },
        credentials: "include",
      });
      const data = await response.json();
      if (response.ok) setStats(data);
    } catch (error) {
      console.error("Gagal mengambil statistik:", error);
    }
  };

  const fetchUpcomingClasses = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("/api/teacher/upcoming-classes", {
        method: "GET",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420"
        },
        credentials: "include",
      });
      const data = await response.json();
      if (response.ok) setUpcomingClasses(data);
    } catch (error) {
      console.error("Gagal mengambil kelas mendatang:", error);
    }
  };

  const fetchTodaySchedule = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("/api/teacher/schedule/today", {
        method: "GET",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420"
        },
        credentials: "include",
      });
      const data = await response.json();
      if (response.ok) setTodaySchedule(data);
    } catch (error) {
      console.error("Gagal mengambil jadwal hari ini:", error);
    }
  };

  const fetchRecentSubmissions = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("/api/teacher/recent-submissions", {
        method: "GET",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420"
        },
        credentials: "include",
      });
      const data = await response.json();
      if (response.ok) setRecentSubmissions(data);
    } catch (error) {
      console.error("Gagal mengambil pengumpulan tugas:", error);
    }
  };

  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      await Promise.all([
        fetchStats(),
        fetchUpcomingClasses(),
        fetchTodaySchedule(),
        fetchRecentSubmissions()
      ]);
      setIsLoading(false);
    };
    loadData();
  }, []);

  return (
    <MainLayoutTeacher>
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
        <header>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
            Dashboard Guru
          </h1>
          <p className="text-slate-500 mt-1 text-lg font-medium">
            Ringkasan aktivitas dan jadwal mengajar Anda.
          </p>
        </header>

        {isLoading ? (
          <div className="flex items-center justify-center h-64">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600"></div>
          </div>
        ) : (
          <>
            <div className="space-y-6">

              <div className="cursor-pointer group" onClick={() => navigate("/teacher/classes")}>
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl hover:border-indigo-300 hover:-translate-y-1 transition-all duration-300 active:scale-[0.98]">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                      <h3 className="text-2xl md:text-3xl font-bold text-slate-900 group-hover:text-indigo-700 transition-colors">
                        Lihat Daftar Mapel
                      </h3>
                      <p className="mt-2 text-sm text-slate-500">Kelola mapel yang Anda ampu, tugas, dan siswa dengan mudah.</p>
                    </div>
                    <div className="p-3 bg-indigo-50 group-hover:bg-indigo-600 rounded-3xl text-indigo-600 group-hover:text-white transition-colors duration-300">
                      <IconAcademic />
                    </div>
                  </div>
                </div>
              </div>

            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

            </div>
          </>
        )}
      </div>
    </MainLayoutTeacher>
  );
}