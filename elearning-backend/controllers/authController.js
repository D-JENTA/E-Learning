...
const signToken = (user) =>
  jwt.sign({ id: user.id_user, role: user.role }, process.env.JWT_SECRET, {
    expiresIn: "1d",
  });
...
const registerStudent = async (req, res) => {
  ...
  try {
    ...
    await Student.create(
      { id_student: newUser.id_user, nis, id_class },
      { transaction: t },
    );
    ...
    if (err.name === "SequelizeUniqueConstraintError") {
      ...
      if (field.includes("nis"))
        return res.status(400).json({ message: "NIS is already used" });
      if (field.includes("nip"))
        return res.status(400).json({ message: "NIP is already used" });
      return res
        .status(400)
        .json({ message: "email is registered, please log in" });
    }
...
const registerTeacher = async (req, res) => {
  ...
  try {
    ...
    setImmediate(() => {
      sendTeacherCredentialsEmail(email, username, tempPassword).catch(
        console.error,
      );
    });
  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      return res.status(200).json({
        message:
          "Login tahap pertama berhasil. Silakan cek email Anda untuk kode OTP.",
...
const resendOtp = async (req, res) => {
  try {
    ...
    if (!user_id && !email) {
      return res
        .status(400)
        .json({ message: "user_id atau email wajib diisi" });
    }
...
const changeUserRole = async (req, res) => {
  ...
  if (!["student", "teacher"].includes(targetRole)) {
    return res.status(400).json({
      message: "Target role tidak valid. Gunakan 'student' atau 'teacher'.",
    });
  }
  if (!newNISORNIP || String(newNISORNIP).trim() === "") {
    return res.status(400).json({
      message:
        targetRole === "teacher"
          ? "NIP baru wajib diisi."
          : "NIS baru wajib diisi.",
    });
  }
  if (targetRole === "student" && !id_class) {
    return res.status(400).json({
      message: "id_class wajib diisi saat mengubah guru menjadi siswa.",
    });
  }
  ...
  try {
    if (targetRole === "teacher") {
      ...
      const hasSubmission = await assignmentStudent.findOne({
        where: { id_student: id },
        transaction: t,
      });
      if (hasSubmission) {
        ...
        return res.status(400).json({
          message:
            "Gagal mengubah peran! Siswa ini sudah memiliki riwayat pengumpulan tugas.",
        });
      }

      await Teacher.create(
        { id_teacher: student.id_student, nip: newNISORNIP },
        { transaction: t },
      );
      await User.update(
        { role: "teacher" },
      ...
      return res
        .status(200)
        .json({ message: "Berhasil mengubah peran Siswa menjadi Guru." });
    }
    ...
    const stillTeaching = await Mapel.findOne({
      where: { id_teacher: id },
      transaction: t,
    });
    if (stillTeaching) {
      ...
      return res.status(400).json({
        message:
          "Gagal mengubah peran! Guru ini masih mengajar mata pelajaran di aplikasi.",
      });
    }

    const createdAssignment = await Assignment.findOne({
      where: { id_teacher: id },
      transaction: t,
    });
    if (createdAssignment) {
      ...
      return res.status(400).json({
        message:
          "Gagal mengubah peran! Guru ini sudah memiliki riwayat membuat tugas.",
      });
    }

    await Student.create(
      { id_student: teacher.id_teacher, nis: newNISORNIP, id_class },
      { transaction: t },
    );
    await User.update(
      { role: "student" },
    ...
    return res
      .status(200)
      .json({ message: "Berhasil mengubah peran Guru menjadi Siswa." });
  } catch (error) {
    if (!t.finished) await t.rollback();
    if (error.name === "SequelizeUniqueConstraintError") {
      return res
        .status(400)
        .json({ message: "NIS/NIP tersebut sudah dipakai user lain." });
    }
    console.error(error);
...
const getStudentByIdClass = async (req, res) => {
  try {
    ...
    if (!id_class)
      return res.status(400).json({ message: "id class is required" });
    ...
    res
      .status(500)
      .json({ message: "server error while get student by id class" });
  }
};
...
const updateProfilePicture = async (req, res) => {
  try {
    ...
    const oldFile = {
      file_public_id: user.profile_public_id,
...
const deleteUser = async (req, res) => {
  try {
    ...
    await user.destroy();
    res.json({
      message: "success delete user by admin",
      data: {
        id_user: user.id_user,
        username: user.username,
        email: user.email,
